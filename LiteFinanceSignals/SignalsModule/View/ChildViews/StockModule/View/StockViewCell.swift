...
class StockViewCell: UITableViewCell {
    ...
    var presenterStock = GetCurrencyName()
    ...
    var flagImage: UIImageView = {
        ...
        image.frame.size = CGSize(width: 35, height: 34)
...
//        image.layer.cornerRadius = image.layer.bounds.width / 2
    ...
    func configure(title: String, time: String, color: [UIColor], new: Bool, difTime: String) {
        DispatchQueue.main.async {
            let newTitle = self.presenterStock.deleteSymbol(text: title)
            
            //let urlFlag = "https://api.litemarkets.com/images/signals/day/\(newTitle).svg"
            let urlFlag = API.createIconURL(icon: newTitle)
        ...
        titleButn.text = presenterStock.setTitleSellButton(color: color).uppercased()
    ...
    override func layoutSubviews() {
        ...
        if LocaleLayout.getLocale() == "ar" || LocaleLayout.getLocale() == "fa" {
            ...
            viewCurrecny.frame = CGRect(x: 0, y: timeLabel.frame.maxY + 7, width: contentView.frame.width, height: 56)
            flagImage.frame = CGRect(x: 10, y: 11, width: 50, height: 34)
